...
# If no recent items, get the latest post
if not recent_items:
    latest_entry = feed.entries[0]  # Get the most recent post
    pub_date = datetime.strptime(latest_entry.published, "%a, %d %b %Y %H:%M:%S %Z")
    pub_date = pub_date.replace(tzinfo=pytz.utc).astimezone(tz)
    item_info = {
